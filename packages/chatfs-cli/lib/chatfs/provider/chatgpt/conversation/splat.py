...
`/backend-api/conversation/{id}` response body).

Output: `<src>.splat/{messages,conversations}/`, or
`<output-dir>/{messages,conversations}/` when an output-dir argument is
given (only the two owned subdirs are cleared/recreated -- sibling
content in an explicit output-dir, e.g. a caller-placed `.data` symlink,
is left alone).
"""
...
def splat(src: Path, base_dir: Path) -> tuple[int, int]:
    """Splat src's conversation into base_dir/{messages,conversations}/.

    Returns (message count, rendered-with-.md count)."""
    import shutil
    ...
    # Clean up previous run's outputs (this function owns these two
    # subdirs, not base_dir itself)
    ...
    return len(messages), rendered_count
