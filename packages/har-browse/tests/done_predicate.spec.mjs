...
test("Done-button click alone drains the events iterator", async ({
  ...
  // Drain with our own 8s ceiling so the failure is localized (an
  // iterator timeout) rather than a generic test timeout from the
  // outer framework.
  const guard = new Promise((_, rej) =>
    setTimeout(() => rej(new Error("iterator did not drain after click")), 8000),
  );
  const messages = await Promise.race([drainMessages(session), guard]);
