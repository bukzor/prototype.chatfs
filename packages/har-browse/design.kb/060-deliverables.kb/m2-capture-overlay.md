...
to signal capture is complete. The event stream is finalized with traffic
from the page's natural behavior. This proves the capture script can inject
controls into pages it doesn't own.
...
- The stream contains the `/api/conversation` response (from initial page load)
