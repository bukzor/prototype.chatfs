...
- [ ] Decide whether `har-browse --headless` should normalize `HeadlessChrome` back to `Chrome` in the User-Agent. Measured 2026-07-27 (`design.kb/040-design.kb/self-identification.kb/headless-changes-the-agent.md`): headless rewrites the UA's browser product under both launchers, so a headless run announces automation on every request. Nothing ships broken — the CLI defaults headful — but the flag is a foot-gun against a real provider, and the offline suite can't catch it because the suite itself runs headless. The call is a policy one: suppressing the tell is the only way to satisfy `unblocked-sessions`' "no more suspicious than the same human uninstrumented", and it is also the one place we would hide something rather than add to it. User decision requested.
