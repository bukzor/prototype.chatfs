...
- [ ] Two broken links, unrelated to any recent har-browse work, noticed via `llm.kb-validate-links .`: `dev.kb/rust-port.kb/commits.kb/1300-retire-node.md` → `../rust-port.md` (likely should be `../../rust-port.md`; `dev.kb/rust-port.md` exists one level further up) and `dev.kb/rust-port.kb/handoffs.kb/CLAUDE.md` → `commits.kb/NNNN-slug.md` (probably an intentional template placeholder, not a real dangling link — worth a human glance either way).
