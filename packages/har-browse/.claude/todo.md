...
- [~] [claude.ai revisits render from persisted React Query IndexedDB cache](todo.kb/2026-07-22-001-claude-ai-revisits-render-from-persisted-React-Query-IndexedDB-cache--so-capture-sees-no-conversation-traffic.md) — revisit captures contain zero conversation traffic (app hydrates from IndexedDB; nothing crosses the network). Implementation landed 2026-07-26: `clearOriginStorage` (indexeddb + cache_storage, pre-goto, cookies preserved) behind `startCapture`/`--clear-origin-storage`, plus unconditional per-session `setCacheDisabled`/`setBypassServiceWorker`; 6 new tests, all six mutation entries confirmed red, `pnpm test` green. Remaining: live verification per provider (ask-first — real browser and login), which is what closes the taskfile's Success Criteria. The flag defaults off pending the taskfile's open question about whether defaulting it on is chatfs-side provider policy.
