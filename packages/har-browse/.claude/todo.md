...
- [x] `--headless` removed 2026-07-28 (user call). It rewrote the UA's browser product to `HeadlessChrome`, so the mode could not ship and the suite that ran in it could not check what does. Normalizing the string back was rejected as the one place we would hide rather than disclose. Record: `design.kb/040-design.kb/self-identification.kb/headless-changes-the-agent.md`. Test-only `host_playwright.mjs` keeps its option.
...
- [ ] Consider `xvfb-run pnpm test` (or a `pnpm test:quiet` script) now that the suite opens real windows: `epipe`, `cli_clear_default` (×3), and `user_agent_client_hints` (×2) launch headful. Not set up — xvfb is not installed here, so this needs `apt install xvfb` first and a verified run before anything goes in the README.
